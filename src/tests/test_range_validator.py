...
def test_detects_percentage_above_100():
    ...
    issues = detect(df)
    assert len(issues) == 1
    assert issues[0]['columns'] == ['success_rate']
...
def test_multiple_domain_columns_each_flagged():
    ...
    issues = detect(df)
    assert len(issues) == 2
    age_issue = [i for i in issues if i['columns'] == ['age']][0]
    rating_issue = [i for i in issues if i['columns'] == ['rating']][0]
